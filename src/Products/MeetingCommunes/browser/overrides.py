...
class MCItemDocumentGenerationHelperView(ItemDocumentGenerationHelperView):
    ...
    def print_all_annexes(self, portal_types=["annex"], filters={}, with_icon=False, with_filename=False):
        ...
        for annex in annexes:
            ...
            if with_icon:
                ...
                file_type_icon = '&nbsp;<img src="{0}/{1}"/>'.format(self.portal.absolute_url(), mime_type.icon_path)
            annex_type_icon = '<img src="{0}/{1}"/>'.format(
                self.portal.absolute_url(), self.real_context.categorized_elements[annex.UID()]["icon_url"]
            )
            # sometimes filename may be None
            if annex.file.filename:
                extension = annex.file.filename.split(".")[-1]
                # escape just in case there is no file extension
                file_info = "&nbsp;({0})".format(safe_unicode(cgi.escape(extension)))
            else:
                file_info = "&nbsp;(???)"

            res.append(
                '<p>{0}&nbsp;<a href="{1}">{2}</a>{3}{4}</p>'.format(
                    annex_type_icon, url, title, file_type_icon, file_info
                )
            )
            if with_filename:
                file_name = safe_unicode(cgi.escape(annex.file.filename))
                res.append("<p><i>{0}</i></p>".format(file_name))
        return "\n".join(res)
    ...
    def print_deliberation(self, xhtmlContents=[], **kwargs):
        ...
        if not xhtmlContents:
            contents = ("motivation", "finance_advices", "decision")
        ...
        return super(MCItemDocumentGenerationHelperView, self).print_deliberation(xhtmlContents, **kwargs)

    def print_formatted_finance_advice(self, finance_used_cases=[], finance_advices_template={}):
        """
        ...
        if not finance_advices_template:
            finance_advices_template = {
                "simple": "<p>Considérant l'avis {type_translated} {by} {adviser} "
                "remis en date du {advice_given_on_localized},</p>",
                "simple_not_given": "<p>Considérant l'avis non rendu par {prefix} {adviser}</p>",
                "legal": "<p>Considérant la transmission du dossier {to} {adviser} "
                "pour avis préalable en date du {item_transmitted_on_localized},</p>"
                "<p>Considérant l'avis {type_translated} {by} {adviser} "
                "remis en date du {advice_given_on_localized},</p>",
                "legal_not_given": "<p>Considérant la transmission du dossier {to} {adviser} "
                "pour avis préalable en date du {item_transmitted_on_localized},</p>"
                "<p>Considérant l'avis non rendu par {prefix} {adviser},</p>",
                "initiative": "<p>Considérant l'avis d'initiative {type_translated} {to} {adviser} "
                "remis en date du {advice_given_on_localized},</p>",
            }
        formatted_finance_advice = ""
        finances_advices = {case: self.print_finance_advice(case) for case in finance_advices_template.keys()}
        for case, advices in finances_advices.items():
            ...
            for advice in advices:
                if self.get_contact_infos(userid=advice["creator_id"])["held_position_label"]:
                    adviser = self.get_contact_infos(userid=advice["creator_id"])["held_position_label"]
                else:
                    adviser = advice["name"]
    ...
    def _get_prefix_for_finance_advice(self, type, advice):
        ...
        if advice["type"] == "not_given" or not self.get_contact_infos(userid=advice["creator_id"])["person"]:
            ...
            for case in cases:
                if case in ["initiative", "legal", "simple", "simple_not_given", "legal_not_given"]:
                    for finance_advice_id in finance_advice_ids:
                        ...
                        if (
                            "hidden_during_redaction" in advice
                            and advice["hidden_during_redaction"]
                            and not show_hidden
                        ):
    ...
    def _getItemAdviceTransmissionDate(self, adviser_id=None, advice=None):
        ...
        if advice:
            return (
                "delay_started_on" in advice
                and advice["delay_started_on"]
    ...
    def print_item_number_ordinal(self, ordinals={"1": "er", "default": "ème"}):
        """
    ...
    def print_item_number_within_category(self, list_types=["normal", "late"], default=""):
        ...
        if self.real_context.hasMeeting() and self.real_context.getListType() in list_types and context_category:
            meeting = self.real_context.getMeeting()
            context_uid = self.real_context.UID()
            count = 0

            for brain in meeting.get_items(
    ...
    def print_completeness_date(self, completeness_action, format="%d/%m/%Y"):
        completeness_changes_adapter = getAdapter(self.real_context, IImioHistory, "completeness_changes")
...
class MCMeetingDocumentGenerationHelperView(MeetingDocumentGenerationHelperView):
    ...
    def get_multiple_level_printing(
        self,
        itemUids,
        list_types=["normal"],
        included_values={},
...
class MCFolderDocumentGenerationHelperView(FolderDocumentGenerationHelperView):
    ...
    def get_finance_advices_stats(self, brains, finance_advice_ids=[], reception_date_wf_state="wait_advices_"):
        ...
        for brain in brains:
            ...
            if not fin_advice_ids:
                continue
            fin_advice_id = list(fin_advice_ids)[0]
            ...
            base_row = {
                ...
                "meeting_date": item.hasMeeting() and item.getMeeting().date.strftime("%d/%m/%Y %H:%M"),
            ...
            row = {
                "reception_date": (
                    filtered_item_history[0]["time"].strftime("%d/%m/%Y %H:%M") if filtered_item_history else ""
                ),
                "advice_date": (
                    advice_infos["advice_given_on"].strftime("%d/%m/%Y %H:%M")
                    if advice_infos["advice_given_on"]
                    else ""
                ),
                "end_advice": "OUI" if advice_infos["advice_given_on"] else "-",
            ...
            if not advice_histories or len(advice_histories) == 1:
                continue
            for i, advice_history in enumerate(advice_histories[1:]):  # skip the first one as it was added previously.
                # We need to do it this way to handle cases when advice is not yet given and there is no advice history.
                advice_history_dict = {
                    item["field_name"]: item["field_content"] for item in advice_history["advice_data"]
                }
                row = {
                    "reception_date": filtered_item_history[i + 1]["time"].strftime("%d/%m/%Y %H:%M"),
                    "advice_date": advice_history["time"].strftime("%d/%m/%Y %H:%M"),
                    "end_advice": "NON",
                    "advice_type": advice_history_dict.get("advice_type"),
                    "comments": advice_history_dict.get("advice_comments"),
                }
                row.update(base_row)
                results.append(row)
    ...
    def _get_given_advice_histories(self, advice):
        ...
        advice_histories = []
        if advice:
            advice_histories = [history for history in getAdapter(advice, IImioHistory, "advice_given").getHistory()]
        # older histories first
        advice_histories.reverse()
        return advice_histories
